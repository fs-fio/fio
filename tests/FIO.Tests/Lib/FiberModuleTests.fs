...
let fiberModuleTests =
    testList "Fiber Module" [

        testList "Fiber.succeed" [
        ...
        testList "Fiber.joinAll" [
            ...
            <| fun (runtime: FIORuntime, values: int list) ->
                let eff = fio {
                    let! fibers =
                        values
                        |> List.map (fun v -> FIO.succeed(v).Fork())
                        |> FIO.collectAll
        ...
        testList "Fiber.awaitAll" [
            ...
            <| fun (runtime: FIORuntime, values: int list) ->
                ...
                let extracted = results |> List.map (fun r ->
                    match r with
                    | Succeeded v -> v
        ...
        testList "Fiber.interruptAwaitAll" [
