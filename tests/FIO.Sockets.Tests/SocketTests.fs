...
let socketTests =
    testList "Socket" [
        ...
        testAllRuntimes "ReceiveBytes fails with non-positive maxBytes" (fun runtime ->
            withTestServer noopHandler (fun port ->
                fio {
                    let! config = SocketConfig.create("127.0.0.1", port)
                    let! socket = SocketClient.connect config
                    let! result =
                        socket.ReceiveBytes(0)
                            .Map(fun _ -> None)
        ...
        testAllRuntimes "ReceiveExactly roundtrip" (fun runtime ->
            withTestServer echoHandler (fun port ->
                fio {
                    ...
                    Expect.equal (Encoding.UTF8.GetString received) "exactdata!" "ReceiveExactly should receive exact bytes"
