    ...
    testList "Types" [
        ...
        testList "SocketError" [
            ...
            testCase "toException creates Exception for other variants" <| fun () ->
                ...
                Expect.stringContains result.Message "peer disconnected"
                    "Exception message should contain error details"
        ]

        testList "SocketConfig" [
            ...
            testCase "builder functions update fields" <| fun () ->
                let config =
                    { Host = "localhost"; Port = 8080
                      ...
                      SendBufferSize = 8192; ReceiveBufferSize = 8192
                      SendTimeout = 30000; ReceiveTimeout = 30000
                      NoDelay = true; LingerEnabled = true; LingerTimeout = 0 }
        ...
        testList "ServerSocketConfig" [
            ...
            testCase "defaultConfig and builders" <| fun () ->
                ...
                let updated2 = ServerSocketConfig.withAddressFamily(Net.Sockets.AddressFamily.InterNetworkV6, config)
                Expect.equal updated2.AddressFamily Net.Sockets.AddressFamily.InterNetworkV6 "Updated AddressFamily"

            testCase "withAcceptedConfig updates field" <| fun () ->
                let config = ServerSocketConfig.defaultConfig
                let socketConfig =
        ...
        testList "SocketPoolConfig" [
            ...
            testCase "pure builders update fields" <| fun () ->
                let poolConfig =
                    { SocketConfig =
                        { Host = "localhost"; Port = 8080
                          AddressFamily = Net.Sockets.AddressFamily.InterNetwork
                          ...
                          SendTimeout = 30000; ReceiveTimeout = 30000
                          NoDelay = true; LingerEnabled = true; LingerTimeout = 0 }
                      MinPoolSize = 0; MaxPoolSize = 10
                      ConnectionLifetime = 300; ValidateOnAcquire = true }
