...
let handlerTests =
    testList "HttpHandler" [
        ...
        testAllRuntimes "unauthorized returns 401" (fun runtime ->
            let resp = runtime.Run(HttpHandler.unauthorized (makeGetRequest "/")).UnsafeSuccess()
        ...
        testAllRuntimes "map transforms response" (fun runtime ->
            let handler =
                ...
                |> HttpHandler.map (fun resp ->
                    HttpResponse.withHeader "X-Mapped" "true" resp)
        ...
        testAllRuntimes "bind chains to new handler" (fun runtime ->
            let handler =
                HttpHandler.text "step1"
                |> HttpHandler.bind (fun _ ->
                    HttpHandler.text "step2")
        ...
        testAllRuntimes "tap runs side effect without changing response" (fun runtime ->
            ...
            let handler =
                HttpHandler.text "hello"
                |> HttpHandler.tap (fun _ -> FIO.attemptExn(fun () -> tapped <- true))
