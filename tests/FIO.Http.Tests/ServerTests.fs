...
[<Tests>]
let serverTests =
    testSequenced (testList "Server" [

        testCase "starts and responds to GET request" <| fun () ->
        ...
        testCase "routes to correct handler" <| fun () ->
            let routes =
        ...
        testCase "returns 404 for unknown path" <| fun () ->
        ...
        testCase "handles JSON response body" <| fun () ->
        ...
        testCase "handles POST with request body" <| fun () ->
            let routes = post "/echo" (fun req ->
                FIO.succeed (Response.okText (req.Body.AsString())))
            withTestHttpServer routes (fun port ->
                use client = new System.Net.Http.HttpClient()
                let content = new System.Net.Http.StringContent("test payload", Encoding.UTF8, "text/plain")
        ...
        testCase "handles text response body" <| fun () ->
