...
let routesTests =
    testList "Routes" [
        ...
        testAllRuntimes "single creates parameterized route" (fun runtime ->
            ...
            let routes = Routes.single pattern (fun params' ->
                match params' with
        ...
        testAllRuntimes "dispatch normalizes trailing slashes" (fun runtime ->
            ...
            let req = { HttpRequest.create HttpMethod.GET "/path/" with
                            PathSegments = ["path"] }
        ...
        testAllRuntimes "withNotFound changes fallback handler" (fun runtime ->
            let routes =
                Routes.empty
                |> Routes.withNotFound (HttpHandler.text "custom 404")
        ...
        testAllRuntimes "add appends route" (fun runtime ->
            let routes =
                Routes.empty
                |> Routes.addRoute (Route.get "/added") (HttpHandler.text "added")
        ...
        testList "TypedRoutes" [

            testAllRuntimes "getInt extracts integer parameter" (fun runtime ->
                let routes = TypedRoutes.getInt ["users"] [] (fun id ->
                    HttpHandler.text $"id={id}")
            ...
            testAllRuntimes "getString extracts string parameter" (fun runtime ->
                let routes = TypedRoutes.getString ["items"] [] (fun name ->
                    HttpHandler.text $"name={name}")
                    
                let resp = dispatchAndRun runtime routes (makeRequest HttpMethod.GET "/items/widget")
