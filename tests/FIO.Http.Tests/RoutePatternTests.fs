...
let routePatternTests =
    testList "RoutePattern" [

        testList "RoutePath" [

            testCase "exact matches same segments" <| fun () ->
                let result = RoutePath.tryMatch (RoutePath.exact ["users"; "list"]) ["users"; "list"]
            ...
            testCase "withInt rejects non-integer" <| fun () ->
                let path = RoutePath.withInt ["users"] []
            ...
            testCase "withInt matches with before and after segments" <| fun () ->
                ...
                match result with
                | Some (params', _) ->
                    Expect.equal (params'.[0] :?> int) 5 "Parsed int"
                | None -> failtest "Expected match"

            testCase "withString matches string parameter" <| fun () ->
                ...
                match result with
                | Some (params', _) ->
                    Expect.equal (params'.[0] :?> string) "readme.md" "Parsed string"
                | None -> failtest "Expected match"

            testCase "withString matches with before and after" <| fun () ->
                ...
                match result with
                | Some (params', _) ->
                    Expect.equal (params'.[0] :?> string) "item1" "Parsed string"
                | None -> failtest "Expected match"
        ]

        testList "RoutePattern matching" [

            testCase "create with GET and exact path matches" <| fun () ->
            ...
            testCase "tryMatch rejects wrong method" <| fun () ->
                let pattern = RoutePattern.get (RoutePath.exact ["users"])
            ...
            testCase "tryMatch rejects wrong path" <| fun () ->
                let pattern = RoutePattern.get (RoutePath.exact ["users"])
            ...
            testCase "method-specific constructors create correct methods" <| fun () ->
        ...
        testList "Route string parsing" [

            testCase "ofString parses GET /path" <| fun () ->
            ...
            testCase "ofString parses route with parameter" <| fun () ->
            ...
            testCase "ofString throws for invalid format" <| fun () ->
                Expect.throws
                    (fun () -> Route.ofString "INVALID" |> ignore)
                    "Invalid format"

            testCase "Route.get/post/put/delete create from string" <| fun () ->
        ...
        testList "RouteOperators" [

            testCase "=> creates route pattern from method and path" <| fun () ->
