...
let private applyMiddleware
    ...
    (request: HttpRequest)
    (runtime: FIO.Runtime.FIORuntime) : HttpResponse =
    let routes =
        Routes.route (Route.get "/test") handler
...
let middlewareTests =
    testList "Middleware" [
        ...
        testAllRuntimes "addHeader adds header to all responses" (fun runtime ->
            ...
            let resp = applyMiddleware mw (HttpHandler.text "hello") (makeGetRequest "/test") runtime
        ...
        testAllRuntimes "after runs effect after handler" (fun runtime ->
            ...
            let mw = Middleware.after (fun _ resp ->
                FIO.attemptExn(fun () -> capturedStatus <- resp.Status))
        ...
        testAllRuntimes "logging calls logger with request" (fun runtime ->
            ...
            let mw = Middleware.logging (fun req ->
                FIO.attemptExn(fun () -> loggedPath <- req.Path))
        ...
        testList "timeout" [
            ...
            testAllRuntimes "returns handler response when fast enough" (fun runtime ->
                ...
                let resp = applyMiddleware mw (HttpHandler.text "fast") (makeGetRequest "/test") runtime
            ...
            testAllRuntimes "timeoutExn convenience works" (fun runtime ->
                ...
                let resp = applyMiddleware mw (HttpHandler.text "ok") (makeGetRequest "/test") runtime
                Expect.equal resp.Status HttpStatusCode.OK "200")
        ]
        ...
        testList "cors" [
            ...
            testAllRuntimes "adds CORS headers for normal request" (fun runtime ->
                let mw = Middleware.cors ["http://example.com"] ["GET"; "POST"] ["Content-Type"]
                let req =
                    makeGetRequest "/test"
                ...
                Expect.equal (HttpResponse.header "Access-Control-Allow-Origin" resp) (Some "http://example.com") "Allow-Origin"
                Expect.equal (HttpResponse.header "Access-Control-Allow-Methods" resp) (Some "GET, POST") "Allow-Methods")
            ...
            testAllRuntimes "returns 403 for disallowed origin" (fun runtime ->
                ...
                let req =
                    makeGetRequest "/test"
                    |> HttpRequest.withHeader "Origin" "http://evil.com"
        ...
        testList "basicAuth" [
            ...
            testAllRuntimes "handles colon in password" (fun runtime ->
                ...
                let mw = Middleware.basicAuth authenticate
                let encoded = Convert.ToBase64String(Text.Encoding.UTF8.GetBytes "admin:pass:word:with:colons")
                let req =
                    makeGetRequest "/test"
                    |> HttpRequest.withHeader "Authorization" $"Basic {encoded}"
        ...
        testList "bearerAuth" [
            ...
            testAllRuntimes "rejects empty token" (fun runtime ->
                ...
                let req =
                    makeGetRequest "/test"
                    |> HttpRequest.withHeader "Authorization" "Bearer "
        ...
        testAllRuntimes "errorHandler catches and converts error to response" (fun runtime ->
            let mw = Middleware.errorHandler (fun (ex: exn) ->
                Response.internalServerErrorText ex.Message)
        ...
        testList "MiddlewareOperators" [
            ...
            testAllRuntimes "+++ composes two middlewares" (fun runtime ->
                let composed =
                    MiddlewareOperators.(+++)
                        (Middleware.addHeader "X-A" "1")
                        (Middleware.addHeader "X-B" "2")
