...
let runWithTimeout (runtime: FIORuntime) (eff: FIO<'R, exn>) : 'R =
    ...
    match
        fiber.Task()
        |> Async.AwaitTask
        |> fun a -> Async.RunSynchronously(a, timeout = 10_000) with
    | Succeeded v -> v
    | Failed e -> failtest $"Effect failed: {e}"
    | Interrupted ex -> failtest $"Interrupted: {ex.Message}"

let makeRequest (method: HttpMethod) (path: string) : HttpRequest =
    HttpRequest.create method path

let makeGetRequest (path: string) : HttpRequest =
...
let withTestHttpServer (routes: Routes<exn>) (action: int -> unit) =
    ...
    Microsoft.AspNetCore.Builder.RunExtensions.Run(app, Microsoft.AspNetCore.Http.RequestDelegate(fun ctx ->
        task {
            try
