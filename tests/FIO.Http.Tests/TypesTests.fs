...
let typesTests =
    testList "Types" [
        ...
        testList "HttpError" [
            ...
            testCase "fromException wraps in GeneralError" <| fun () ->
                let exn = Exception "test"
                let err = HttpError.fromException exn

                match err with
                | GeneralError e ->
                    Expect.isTrue (Object.ReferenceEquals(e, exn))
                        "Should wrap same exception reference"
                | _ ->
                    failtest "Expected GeneralError"

            testCase "toException unwraps GeneralError" <| fun () ->
                let original = Exception "test"
                let err = GeneralError original
                let result = HttpError.toException err

                Expect.isTrue (Object.ReferenceEquals(result, original))
                    "Should return same exception reference"

            testCase "toException creates Exception for other variants" <| fun () ->
                let err = InvalidRoute "/bad"
                let result = HttpError.toException err

                Expect.stringContains result.Message "/bad"
                    "Exception message should contain error details"

            testCase "ToString produces readable messages" <| fun () ->
        ...
        testList "HttpMethod" [
            ...
            testCase "ToString returns correct string for standard methods" <| fun () ->
            ...
            testCase "fromString parses all standard methods" <| fun () ->
        ...
        testList "HttpRequest" [
            ...
            testCase "withQueryParam appends to existing key" <| fun () ->
                let req =
                    HttpRequest.create HttpMethod.GET "/search"
            ...
            testCase "withHeader adds header" <| fun () ->
                let req =
            ...
            testCase "withBody sets body" <| fun () ->
                let req =
            ...
            testCase "withMetadata adds typed metadata" <| fun () ->
                let req =
                    HttpRequest.create HttpMethod.GET "/"
        ...
        testList "HttpResponse" [
            ...
            testCase "withHeader throws for empty name" <| fun () ->
                Expect.throws
                    (fun () ->
                        HttpResponse.create HttpStatusCode.OK
            ...
            testCase "withHeader throws for invalid characters" <| fun () ->
                Expect.throws
                    (fun () ->
                        HttpResponse.create HttpStatusCode.OK
        ...
        testList "ServerConfig" [

            testCase "defaultConfig has expected values" <| fun () ->
