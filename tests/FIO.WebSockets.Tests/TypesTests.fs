...
let typesTests =
    testList "Types" [
        ...
        testList "WsError" [
            ...
            testCase "fromException maps TimeoutException to TimeoutError" <| fun () ->
            ...
            testCase "fromException maps WebSocketException to GeneralError" <| fun () ->
            ...
            testCase "fromException maps generic Exception to GeneralError" <| fun () ->
            ...
            testCase "toException maps TimeoutError to TimeoutException" <| fun () ->
            ...
            testCase "toException maps GeneralError to WebSocketException" <| fun () ->
                let err = GeneralError "general msg"
                let exn = WsError.toException err

                Expect.isTrue (exn :? WebSocketException) "Should be WebSocketException"

            testCase "toException maps other variants to Exception with message" <| fun () ->
        ...
        testList "WebSocketConfig" [
            ...
            testCase "builder functions update correct fields" <| fun () ->
                let config = WebSocketConfig.defaultConfig
        ...
        testList "WebSocketFrame" [

            testCase "Text frame construction" <| fun () ->
                let frame = Text "hello"
                match frame with
                | Text s -> Expect.equal s "hello" "Text content"
                | _ -> failtest "Expected Text frame"

            testCase "Binary frame construction" <| fun () ->
            ...
            testCase "Close frame construction" <| fun () ->
        ...
        testList "WebSocketMessage" [

            testCase "Frame wraps WebSocketFrame" <| fun () ->
                let msg = Frame(Text "hello")
                match msg with
                | Frame(Text s) -> Expect.equal s "hello" "Frame text content"
                | _ -> failtest "Expected Frame message"

            testCase "ConnectionClosed carries optional status" <| fun () ->
                ...
                match msg2 with
                | ConnectionClosed(status, _) ->
                    Expect.isNone status "Status should be None"
                | _ -> failtest "Expected ConnectionClosed"
        ]

        testList "WebSocketPoolConfig" [

            testCase "create sets min and max" <| fun () ->
