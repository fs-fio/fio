...
let private progressMilestones (totalRuns: int) =
    ...
    |> List.map (fun pct ->
        let run =
            Math.Ceiling(float pct / 100.0 * float totalRuns)
            |> int
            |> max 1
        run, pct)
...
let private runBenchmark (runtime: FIORuntime, totalRuns: int, warmupRuns: int, outputMode: OutputMode, config: BenchmarkConfig) =
    ...
    let rec executeBenchmark (eff: FIO<int64, exn>, proc: Process) =
        task {
            ...
            for currentRun in 1..totalRuns do
                if isDetailed then
                    printfn $"[bench/run] %s{benchmarkName} | runtime=%s{runtimeName} | run=%i{currentRun}/%i{totalRuns}"
                ...
                if isDetailed then
                    printfn $"[bench/run-done] %s{benchmarkName} | runtime=%s{runtimeName} | run=%i{currentRun}/%i{totalRuns} | time=%i{executionTime}ms"
                elif isConcise then
                    match checkpoints |> Map.tryFind currentRun with
                    | Some pct ->
                        let avgExecutionTime = float executionTimeSum / float currentRun
        
                        printfn $"[bench/progress] %s{benchmarkName} | runtime=%s{runtimeName} | %i{pct}%% (%i{currentRun}/%i{totalRuns}) | last=%i{executionTime}ms | avg=%.2f{avgExecutionTime}ms"
                    | None ->
                        ()
    ...
    task {
        ...
        if warmupRuns > 0 then
            ...
            for warmupRun in 1..warmupRuns do
                if isDetailed then
                    printfn $"[bench/warmup-run] %s{benchmarkName} | runtime=%s{runtimeName} | run=%i{warmupRun}/%i{warmupRuns}"
        ...
        printfn $"[bench/done] %s{configText} | runtime=%s{runtimeName} | median=%.2f{medianExecution}ms | p95=%.2f{p95Execution}ms"
...
let private runBenchmarkForSelection (runtimeSelection: RuntimeSelection, totalRuns: int, warmupRuns: int, outputMode: OutputMode, config: BenchmarkConfig) =
    task {
...
let internal run args =
    task {
        ...
        if args.WarmupRuns < 0 then
            invalidArg (nameof args.WarmupRuns) $"BenchmarkRunner: WarmupRuns must be >= 0. WarmupRuns = %i{args.WarmupRuns}"

        let outputMode =
            if args.Quiet then
                Quiet
            elif args.Detailed then
                Detailed
            else
                Concise
        ...
        let completedComparisons = ResizeArray<BenchmarkConfig * BenchmarkResult list> configs.Length

        let printDeferredComparisons () =
            let hasComparisonToPrint =
                completedComparisons
                |> Seq.exists (fun (_, results) -> results.Length > 1)
        ...
        try
            for config in configs do
                ...
                for runtimeSelection in args.Runtimes do
                    let! result = runBenchmarkForSelection(runtimeSelection, args.Runs, args.WarmupRuns, outputMode, config)
