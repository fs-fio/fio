...
let private bigEff (actors: Actor list, roundCount, msg, timerChan, startChan) : FIO<unit, exn> =
    fio {
        ...
        let mutable currentEff = actorEff(actors.Head, currentMsg, roundCount, timerChan, startChan)
       
        for actor in actors.Tail do
            currentMsg <- currentMsg + 10
            currentEff <- actorEff(actor, currentMsg, roundCount, timerChan, startChan)
                          <&&> currentEff
        
        return! currentEff
    }

let bigBenchmark config : FIO<int64, exn> =
    fio {
