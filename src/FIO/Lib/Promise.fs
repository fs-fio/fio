...
type Promise<'R, 'E> internal () =
    ...
    member _.IsDone() : FIO<bool, 'E> = FIO.succeed tcs.Task.IsCompleted
...
module Promise =
    ...
    /// <param name="res">Success value.</param>
    let inline succeedExn<'R> (res: 'R) : FIO<Promise<'R, exn>, exn> = succeed<'R, exn> (res, id)
    ...
    /// <param name="err">Error value.</param>
    let inline failExn<'R> (err: exn) : FIO<Promise<'R, exn>, exn> = fail<'R, exn> (err, id)
