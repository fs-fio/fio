...
type FIO<'R, 'E> with
    ...
    member inline this.Map<'R1> (mapper: 'R -> 'R1) : FIO<'R1, 'E> =
    ...
    member inline this.MapError<'E1> (mapper: 'E -> 'E1) : FIO<'R, 'E1> =
    ...
    member inline this.MapBoth<'R1, 'E1> (successMapper: 'R -> 'R1) (errorMapper: 'E -> 'E1) : FIO<'R1, 'E1> =
    ...
    member inline this.MapAttempt<'R1> (mapper: 'R -> 'R1) (onError: exn -> 'E) : FIO<'R1, 'E> =
        this.FlatMap(fun res -> FIO.attempt (fun () -> mapper res) onError)
    ...
    member inline this.As<'R1> (res: 'R1) : FIO<'R1, 'E> =
        this.Map(fun _ -> res)
    ...
    member inline this.AsLeft<'B> () : FIO<Choice<'R, 'B>, 'E> =
        this.Map Choice1Of2

    /// <summary>Wraps this effect's success value in <c>Choice2Of2</c>.</summary>
    /// <typeparam name="'A">The phantom left-branch type of the resulting choice; never inhabited.</typeparam>
    /// <returns>An effect that completes with <c>Choice2Of2</c> applied to the original success value.</returns>
    member inline this.AsRight<'A> () : FIO<Choice<'A, 'R>, 'E> =
        this.Map Choice2Of2
    ...
    member inline this.AsSome () : FIO<'R option, 'E> =
        this.Map Some
    ...
    member inline this.AsLeftError<'B> () : FIO<'R, Choice<'E, 'B>> =
    ...
    member inline this.AsRightError<'A> () : FIO<'R, Choice<'A, 'E>> =
    ...
    member inline this.AsSomeError<'R, 'E> () : FIO<'R, 'E option> =
        this.MapError Some
    ...
    member inline this.Result<'E1> () : FIO<Result<'R, 'E>, 'E1> =
    ...
    member inline this.Option<'E1> () : FIO<'R option, 'E1> =
    ...
    member inline this.Choice<'E1> () : FIO<Choice<'R, 'E>, 'E1> =
        this.Map(Choice1Of2).CatchAll(fun err ->
            FIO.succeed (Choice2Of2 err))
    ...
    member inline this.Flip () : FIO<'E, 'R> =
        this.Result().FlatMap(fun result ->
            match result with
            | Ok res -> FIO.fail res
            | Error err -> FIO.succeed err)
    ...
    member inline this.Tap<'R1> (effOnSuccess: 'R -> FIO<'R1, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.TapError<'R1> (effOnError: 'E -> FIO<'R1, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.TapBoth<'R1, 'R2> (onSuccess: 'R -> FIO<'R2, 'E>) (onError: 'E -> FIO<'R1, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.Debug (?message: string) : FIO<'R, 'E> =
        let message = defaultArg message "Debug"
        this.Tap(fun res ->
            (FIO.attempt (fun () -> printfn "%s: %A" message res) id)
                .CatchAll(fun ex ->
    ...
    member inline this.DebugError (?message: string) : FIO<'R, 'E> =
    ...
    member inline this.OrElse<'E1> (eff: FIO<'R, 'E1>) : FIO<'R, 'E1> =
    ...
    member inline this.OrElseSucceed<'E1> (result: 'R) : FIO<'R, 'E1> =
    ...
    member inline this.OrElseFail<'E1> (error: 'E1) : FIO<'R, 'E1> =
    ...
    /// <typeparam name="'R1">The success result type of the fallback effect.</typeparam>
    ...
    member inline this.OrElseEither<'R1, 'E1> (eff: FIO<'R1, 'E1>) : FIO<Choice<'R, 'R1>, 'E1> =
    ...
    member inline this.CatchSome (func: 'E -> FIO<'R, 'E> option) : FIO<'R, 'E> =
    ...
    member inline this.OrInterrupt<'E1> (toMessage: 'E -> string) : FIO<'R, 'E1> =
        this.CatchAll(fun err ->
    ...
    member inline this.FilterOrFail (predicate: 'R -> bool) (error: 'E) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            if predicate res then FIO.succeed res else FIO.fail error)
    ...
    member inline this.FilterOrElse (predicate: 'R -> bool) (fallback: FIO<'R, 'E>) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            if predicate res then FIO.succeed res else fallback)
    ...
    member inline this.FilterOrElseWith (predicate: 'R -> bool) (fallback: 'R -> FIO<'R, 'E>) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            if predicate res then FIO.succeed res else fallback res)
    ...
    member inline this.FilterOrInterrupt (predicate: 'R -> bool) (message: string) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            if predicate res then FIO.succeed res
    ...
    member inline this.Reject (func: 'R -> 'E option) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            match func res with
            | Some err -> FIO.fail err
            | None -> FIO.succeed res)
    ...
    member inline this.RejectFIO (func: 'R -> FIO<'E, 'E> option) : FIO<'R, 'E> =
        this.FlatMap(fun res ->
            match func res with
    ...
    /// <typeparam name="'R1">The extracted success result type produced by the partial function.</typeparam>
    ...
    member inline this.Collect<'R1> (error: 'E) (func: 'R -> 'R1 option) : FIO<'R1, 'E> =
        this.FlatMap(fun res ->
            match func res with
    ...
    /// <typeparam name="'R1">The extracted success result type produced by the partial function's effect.</typeparam>
    /// <param name="error">The typed error to fail with when <paramref name="pf"/> does not match.</param>
    ...
    member inline this.CollectFIO<'R1> (error: 'E) (func: 'R -> FIO<'R1, 'E> option) : FIO<'R1, 'E> =
        this.FlatMap(fun res ->
            match func res with
            | Some eff -> eff
            | None -> FIO.fail error)
    ...
    member inline this.Apply<'R1> (eff: FIO<'R -> 'R1, 'E>) : FIO<'R1, 'E> =
    ...
    member inline this.ApplyError<'E1> (eff: FIO<'R, 'E -> 'E1>) : FIO<'R, 'E1> =
    ...
    member inline this.Zip<'R1> (eff: FIO<'R1, 'E>) : FIO<'R * 'R1, 'E> =
    ...
    member inline this.ZipError<'E1> (eff: FIO<'R, 'E1>) : FIO<'R, 'E * 'E1> =
    ...
    member inline this.ZipRight<'R1> (eff: FIO<'R1, 'E>) : FIO<'R1, 'E> =
    ...
    member inline this.ZipLeft<'R1> (eff: FIO<'R1, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.ZipPar<'R1> (eff: FIO<'R1, 'E>) : FIO<'R * 'R1, 'E> =
    ...
    member inline this.ZipParError (eff: FIO<'R, 'E>) : FIO<'R, 'E * 'E> =
    ...
    member inline this.ZipParRight<'R1> (eff: FIO<'R1, 'E>) : FIO<'R1, 'E> =
    ...
    member inline this.ZipParLeft<'R1> (eff: FIO<'R1, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.Fold<'R1, 'E1> (onError: 'E -> 'R1) (onSuccess: 'R -> 'R1) : FIO<'R1, 'E1> =
    ...
    member inline this.FoldFIO<'R1> (onError: 'E -> FIO<'R1, 'E>) (onSuccess: 'R -> FIO<'R1, 'E>) : FIO<'R1, 'E> =
    ...
    member inline this.OnDone (onError: 'E -> FIO<unit, 'E>) (onSuccess: 'R -> FIO<unit, 'E>) : FIO<unit, 'E> =
    ...
    member inline this.RetryOrElse<'E1> (maxAttempts: int) (orElse: 'E -> FIO<'R, 'E1>) (onEachRetry: 'E * int * int -> FIO<unit, 'E1>) : FIO<'R, 'E1> =
            ...
            let rec loop attempt =
                this.CatchAll(fun err ->
                    if attempt >= maxAttempts then
                        ...
                        onEachRetry(err, attempt, maxAttempts)
    ...
    member inline this.Retry (maxAttempts: int) (onEachRetry: 'E * int * int -> FIO<unit, 'E>) : FIO<'R, 'E> =
    ...
    member inline this.RetryUntil (predicate: 'E -> bool) : FIO<'R, 'E> =
        this.RetryUntilFIO(fun err -> FIO.succeed (predicate err))
    ...
    member inline this.RetryUntilEquals (error: 'E) : FIO<'R, 'E> =
    ...
    member this.RetryUntilFIO (predicate: 'E -> FIO<bool, 'E>) : FIO<'R, 'E> =
        let rec loop () =
            this.CatchAll(fun err -> (predicate err).FlatMap(fun stop ->
                if stop then FIO.fail err else loop ()))
        loop ()
    ...
    /// <remarks>The loop is unbounded; combining with <c>Retry(maxAttempts)</c> or <c>RetryOrElse</c> is redundant because the cap is swallowed. Pair with <c>Timeout</c> when a wall-clock bound is needed.</remarks>
    member inline this.RetryWhile (predicate: 'E -> bool) : FIO<'R, 'E> =
        this.RetryWhileFIO(fun err -> FIO.succeed (predicate err))
    ...
    /// <remarks>The loop is unbounded; combining with <c>Retry(maxAttempts)</c> or <c>RetryOrElse</c> is redundant because the cap is swallowed. Pair with <c>Timeout</c> when a wall-clock bound is needed. Equality uses <c>Unchecked.equals</c> so no <c>equality</c> constraint is required.</remarks>
    member inline this.RetryWhileEquals (error: 'E) : FIO<'R, 'E> =
    ...
    /// <returns>An effect that completes with the first success, or fails with the error for which <paramref name="predicate"/> returned <c>false</c>.</returns>
    /// <remarks>The loop is unbounded; combining with <c>Retry(maxAttempts)</c> or <c>RetryOrElse</c> is redundant because the cap is swallowed. Pair with <c>Timeout</c> when a wall-clock bound is needed. Failures from <paramref name="predicate"/> propagate as the effect's error.</remarks>
    member this.RetryWhileFIO (predicate: 'E -> FIO<bool, 'E>) : FIO<'R, 'E> =
        let rec loop () =
            this.CatchAll(fun err -> (predicate err).FlatMap(fun cont ->
                if cont then loop () else FIO.fail err))
        loop ()
    ...
    member this.Eventually<'E1> () : FIO<'R, 'E1> =
        let rec loop () =
    ...
    member this.RepeatN (n: int) : FIO<'R, 'E> =
        if n <= 1 then
            this
        else
    ...
    member inline this.RepeatUntil (predicate: 'R -> bool) : FIO<'R, 'E> =
        this.RepeatUntilFIO(fun res -> FIO.succeed (predicate res))
    ...
    member inline this.RepeatUntilEquals (result: 'R) : FIO<'R, 'E> =
        this.RepeatUntil(fun res -> Unchecked.equals res result)
    ...
    member this.RepeatUntilFIO (predicate: 'R -> FIO<bool, 'E>) : FIO<'R, 'E> =
        let rec loop () =
            this.FlatMap(fun res -> (predicate res).FlatMap(fun stop ->
                if stop then FIO.succeed res else loop ()))
        loop ()
    ...
    /// <remarks>This effect always runs at least once. The loop is unbounded; pair with <c>Timeout</c> when a cap is needed.</remarks>
    member inline this.RepeatWhile (predicate: 'R -> bool) : FIO<'R, 'E> =
        this.RepeatWhileFIO(fun res -> FIO.succeed (predicate res))
    ...
    /// <remarks>This effect always runs at least once. The loop is unbounded; pair with <c>Timeout</c> when a cap is needed. Equality uses <c>Unchecked.equals</c> so no <c>equality</c> constraint is required.</remarks>
    member inline this.RepeatWhileEquals (value: 'R) : FIO<'R, 'E> =
        this.RepeatWhile(fun res -> Unchecked.equals res value)
    ...
    /// <returns>An effect that completes with the first success value for which <paramref name="predicate"/> returns <c>false</c>.</returns>
    /// <remarks>This effect always runs at least once. The loop is unbounded; pair with <c>Timeout</c> when a cap is needed. Failures from <paramref name="predicate"/> propagate as the effect's error.</remarks>
    member this.RepeatWhileFIO (predicate: 'R -> FIO<bool, 'E>) : FIO<'R, 'E> =
        let rec loop () =
            this.FlatMap(fun res -> (predicate res).FlatMap(fun cont ->
                if cont then loop () else FIO.succeed res))
        loop ()
    ...
    member inline this.Delay (duration: TimeSpan) (onError: exn -> 'E) : FIO<'R, 'E> =
    ...
    member this.Timeout (duration: TimeSpan) (onError: exn -> 'E) : FIO<'R option, 'E> =
        let timeoutEff =
    ...
    member this.TimeoutFail (timeoutError: 'E) (duration: TimeSpan) (onError: exn -> 'E) : FIO<'R, 'E> =
        let timeoutEff =
            (FIO.sleep duration onError)
    ...
    /// <typeparam name="'R1">The result type produced by both the success mapper and the default value.</typeparam>
    ...
    member this.TimeoutTo<'R1> (defaultValue: 'R1) (onSuccess: 'R -> 'R1) (duration: TimeSpan) (onError: exn -> 'E) : FIO<'R1, 'E> =
        let timeoutEff =
            (FIO.sleep duration onError)
    ...
    member inline this.Timed (onError: exn -> 'E) : FIO<TimeSpan * 'R, 'E> =
    ...
    member this.Race (eff: FIO<'R, 'E>) : FIO<'R, 'E> =
        FIO.suspend <| fun () ->
            ...
            let signal (won: bool) (fiber: Fiber<'R, 'E>) =
                fiber.Await().FlatMap(fun result ->
                    match result with
                    | Succeeded _
                    | Failed _ -> resultChan.Send(won).Map(fun _ -> ())
                ...
                eff.Fork().FlatMap(fun fiber2 ->
                    (signal true fiber1).Fork().FlatMap(fun _ ->
                        (signal false fiber2).Fork().FlatMap(fun _ ->
                            resultChan.Receive().FlatMap(fun winnerIsFirst ->
                                let winner, loser =
    ...
    /// <typeparam name="'R1">The success result type of the right-hand racer.</typeparam>
    /// <param name="eff">The effect to race against this one.</param>
    ...
    member this.RaceEither<'R1> (eff: FIO<'R1, 'E>) : FIO<Choice<'R, 'R1>, 'E> =
    ...
    member this.RaceFirstSuccess (eff: FIO<'R, 'E>) : FIO<'R, 'E> =
        FIO.suspend <| fun () ->
            let resultChan = Channel<Result<'R * bool, 'E>>()

            let signal (isFirst: bool) (fiber: Fiber<'R, 'E>) =
                fiber.Await().FlatMap(fun result ->
                    match result with
                    | Succeeded res -> resultChan.Send(Ok(res, isFirst)).Map(fun _ -> ())
                    | Failed err -> resultChan.Send(Error err).Map(fun _ -> ())
                    | Interrupted _ -> FIO.unit ())
            ...
            this.Fork().FlatMap(fun fiber1 ->
                eff.Fork().FlatMap(fun fiber2 ->
                    (signal true fiber1).Fork().FlatMap(fun _ ->
                        (signal false fiber2).Fork().FlatMap(fun _ ->
                            resultChan.Receive().FlatMap(fun first ->
                                match first with
                                | Ok(res, isFirst) ->
                                    ...
                                    loser
                                        .Interrupt(ExplicitInterrupt, "Lost race")
                                        .CatchAll(fun _ -> FIO.unit ())
                                        .FlatMap(fun () -> FIO.succeed res)
                                | Error _ ->
                                    resultChan.Receive().FlatMap(fun second ->
                                        match second with
                                        | Ok(res, _) -> FIO.succeed res
