...
type WebSocketPool =
    {
        ...
        /// </summary>
        mutable Available: (FIO.WebSockets.WebSocket * DateTime) list
        /// <summary>
...
module WebSocketPool =
    ...
    /// </summary>
    let private validateSocket (ws: FIO.WebSockets.WebSocket) : FIO<bool, WsError> =
        fio {
            let! state = ws.State()
    ...
    let rec acquire (pool: WebSocketPool) : FIO<FIO.WebSockets.WebSocket, FIO.WebSockets.WsError> =
        fio {
            if pool.Closed then
    ...
    let releaseToPool (ws: FIO.WebSockets.WebSocket) (pool: WebSocketPool) : FIO<unit, WsError> =
        (fio {
    ...
    let withConnection<'R>
        (action: FIO.WebSockets.WebSocket -> FIO<'R, WsError>)
        (pool: WebSocketPool)
        : FIO<'R, WsError> =
        FIO.acquireRelease(
            acquire pool,
